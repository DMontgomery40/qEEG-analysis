...
def get_completed_runs_by_patient_label() -> dict[str, list[tuple[str, str]]]:
    ...
    with session_scope() as session:
        ...
        for patient in patients:
            ...
            completed_runs = [r for r in patient.runs if r.status == "complete"]
...
def stage_artifacts(dry_run: bool = False) -> None:
    ...
    for patient_label, runs in runs_by_label.items():
            ...
            if stage1_dir.exists():
                ...
                if data_pack.exists():
                    files_to_copy.append(
                        (data_pack, dest_dir / "stage-1" / "_data_pack.json")
                    )
                ...
                if vision_transcript.exists():
                    files_to_copy.append(
                        (
                            vision_transcript,
                            dest_dir / "stage-1" / "_vision_transcript.md",
                        )
                    )
        ...
        if patient_copied and not dry_run:
...
def main():
    parser = argparse.ArgumentParser(
        description="Stage council artifacts for portal sync"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
