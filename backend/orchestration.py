...
def final_export_delivery_gaps_for_run(
    session: storage.Session,
    run: storage.Run,
    *,
    progress: dict[str, Any] | None = None,
) -> list[str]:
    return run_downstream_delivery_gaps(
        run,
        progress=summarize_run_progress(run) if progress is None else progress,
        artifacts=storage.list_artifacts(session, run.id),
        require_selected_final_draft=True,
    )


def patient_facing_delivery_gaps_for_run(
    session: storage.Session,
    run: storage.Run,
    *,
    progress: dict[str, Any] | None = None,
...
def _prefer_run_id(
    runs: list[storage.Run], preferred_run_id: str | None
) -> list[storage.Run]:
    if not preferred_run_id:
        return runs
    return [run for run in runs if run.id == preferred_run_id]


def choose_patient_facing_ready_run(
    session: storage.Session,
    *,
    patient_id: str,
    ...
    for run in runs:
        if (run.status or "") == "complete" and not patient_facing_delivery_gaps_for_run(
            session,
            run,
...
def choose_final_export_ready_run(
    session: storage.Session,
    *,
    patient_id: str,
    preferred_run_id: str | None = None,
    ...
    runs = _prefer_run_id(storage.list_runs(session, patient_id), preferred_run_id)
    for run in runs:
        if (run.status or "") == "complete" and not final_export_delivery_gaps_for_run(
            session,
            run,
            progress=(run_progress_by_id or {}).get(run.id),
        ):
...
def build_patient_orchestration_detail(
    ...
    latest_patient_facing_ready_run = choose_patient_facing_ready_run(
        session, patient_id=patient.id, run_progress_by_id=run_progress_by_id
    )
    latest_export_ready_run = choose_final_export_ready_run(
        session, patient_id=patient.id, run_progress_by_id=run_progress_by_id
    )
    ...
    return {
        ...
        "actions": {
            ...
            "export_council_artifacts": {
                ...
                "reason": ""
                if latest_export_ready_run is not None
