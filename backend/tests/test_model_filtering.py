...
def test_model_visible_in_ui_filters_legacy_provider_versions(temp_data_dir):
    ...
    # OpenAI: keep only 5.5 families, with low reasoning when effort is explicit.
    ...
    assert _model_visible_in_ui("gpt-5.2") is False
    assert _model_visible_in_ui("openai/gpt-5.2-codex") is False
    assert _model_visible_in_ui("gpt-5.3-codex-high") is False
    assert _model_visible_in_ui("gpt-5.3-xhigh") is False
    assert _model_visible_in_ui("gpt-5.4") is False
    assert _model_visible_in_ui("gpt-5.4-pro") is False
    assert _model_visible_in_ui("gpt-5.5") is True
    assert _model_visible_in_ui("openai/gpt-5.5-low") is True
    assert _model_visible_in_ui("openai/gpt-5.5-xhigh") is False
...
def test_models_endpoint_reports_configured_availability_from_real_discovery(monkeypatch):
    ...
    assert configured["gpt-5.5"]["available"] is True
    assert configured["gpt-5.5"]["resolved_discovered_id"] == "openai/gpt-5.5"
    assert configured["gemini-3.1-flash-lite-preview"]["available"] is True
    assert (
