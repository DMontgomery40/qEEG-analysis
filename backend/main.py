...
from .orchestration import (
    ...
    run_is_peer_reviewed_for_downstream,
...
async def _auto_generate_cathode_video_for_run(
    ...
    with storage.session_scope() as session:
        ...
        source_choice = choose_cathode_source_artifact(
            session, patient_id=patient.id, preferred_run_id=run_id
        )
...
async def run_patient_action(
    ...
    if action == "prepare_cathode_handoff":
        ...
        with storage.session_scope() as session:
            ...
            if not run_is_peer_reviewed_for_downstream(
                source_run,
                progress=summarize_run_progress(source_run),
                artifacts=storage.list_artifacts(session, source_run.id),
            ):
                raise HTTPException(
                    status_code=409,
